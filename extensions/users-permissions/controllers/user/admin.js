...
module.exports = {
  ...
  async create(ctx) {
    ...
    const sanitizedBody = pm.pickPermittedFieldsOf(body, {
      subject: userModel,
    });
    ...
    try {
      ...
      console.log(`null, formatError(error)`);
  ...
  async update(ctx) {
    ...
    const sanitizedData = pm.pickPermittedFieldsOf(body, {
      subject: pm.toSubject(user),
    });
    const updateData = _.omit(
      { ...sanitizedData, updated_by: admin.id },
      "created_by"
    );
